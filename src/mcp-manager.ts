...
import { ServerNotFoundError } from './errors'; // Removed McpManagerError
import { ConsoleLogger } from './logger'; // Removed defaultLogger
import { SchemaManager } from './schema-manager';
import { ServerRegistry } from './server-registry';
import { ToolInvoker } from './tool-invoker';
import {
  // LoggerInterface, // Removed import
  McpConnection,
  McpManagerEvent,
  // McpManagerOptions, // Removed unused import
  McpServerConfig,
  // StorageAdapter, // Removed import
  StreamUpdate,
  ToolCallResult,
  ToolDefinition,
} from './types';
// Import LoggerInterface from where it's defined
...
export class McpManager extends EventEmitter {
  ...
  // private storageAdapter?: StorageAdapter; // Removed storage adapter property
  ...
  constructor(/* options?: McpManagerOptions */) { // Removed options for now
    super();
    // this.storageAdapter = options?.storageAdapter; // Removed storage adapter assignment
    // Ensure logger exists, default to ConsoleLogger if options.logger is undefined
    this.logger = new ConsoleLogger(); // Use internal logger directly

    // Instantiate components, passing dependencies
    // ServerRegistry constructor is now (storageDir?: string, logger?: LoggerInterface)
    this.serverRegistry = new ServerRegistry(undefined, this.logger); // Pass undefined for storageDir, then logger
    // ConnectionManager now only needs ServerRegistry and Logger
